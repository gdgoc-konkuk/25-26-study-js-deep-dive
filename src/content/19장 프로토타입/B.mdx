...
2. false가 출력된다.
3. undefined가 출력된다.
4. TypeError: obj.hasOwnProperty is not a function이 발생한다.
5. 아무것도 출력되지 않는다.
--------
2. 다음 코드의 결과값 (A)와 (B)를 순서대로 짝지은 것은?
```javascript
function Parent() {}
...
1. **4번** (TypeError: obj.hasOwnProperty is not a function)
