...
- 평가 과정에서 실행 컨텍스트가 생성되고, 해당 실행 컨텍스트가 관리하는 스코프(실행 컨텍스트가 참조하는 _LexicalEnvironment_/_EnvironmentRecord_ 안에 바인딩이 저장된다는 뜻)가 구성됩니다.
...
1. 함수 코드 평가
   ...
   - 함수 내부에서 지역 변수처럼 사용할 수 있는 `arguments` 객체가 생성되어 실행 컨텍스트가 관리하는 스코프에 등록되고[^2], `this` 바인딩도 결정됩니다.
     [^2]: 엄밀히 말하면, `arguments`는 현재 함수 실행 컨텍스트의 *LexicalEnvironment*가 가리키는 _FunctionEnvironmentRecord_ 안에 binding으로 존재하며, 이 레코드를 통해 식별자로 resolve됩니다. 즉 **스코프에 있다**라고 볼 수 있지만, 실행 컨텍스트 자체에 저장되는 것은 아닙니다.
1. 함수 코드 실행
...
- 함수 선언문으로 정의된 함수 `foo()`는 함수 이름과 동일한 이름의 식별자를 _객체 환경 레코드에 바인딩된 `BindingObject`_ 를 통해 전역 객체에 키로 등록하고, 생성된 함수 객체를 즉시 할당합니다.
...
<span style={{ fontSize: '1.5rem' }}>1<span style={{ fontWeight: 'bold', marginLeft: '0.75rem' }}>평가와 실행</span></span>
<br />
아래 코드의 평가 단계에서, 각 `x`와 `y`에 대해 무슨 일이 일어날까요?
또한, 실행 단계에서 `console.log(x)`와 `console.log(y)`의 출력 결과는 무엇일까요?
실행 컨텍스트와 렉시컬 환경 관점에서 설명해봅시다.
...
- 평가 단계
  - `var x`
    - 전역 환경 레코드의 객체 환경 레코드에서 선언 + `undefined`로 초기화
  - `let y`, `const z`
    - 전역 환경 레코드의 선언적 환경 레코드에 바인딩 + TDZ
- 실행 단계
  - `console.log(x)`
    - 전역 환경에서 식별자 `x`를 검색
    - 값 `undefined`를 출력
  - `console.log(y)`
    - 전역 환경에서 식별자 `y`를 검색
    - 선언은 되어있지만 TDZ에 있어서 `ReferenceError` 발생
  - `console.log(z)` - 실행 안 됨 - 만약 실행된다면? - 전역 환경에서 식별자 `z`를 검색 - 선언은 되어있지만 TDZ에 있어서 `ReferenceError` 발생
...
<div style={{ border: '1px solid #ccc', padding: '1.25rem', borderRadius: '0.5rem', marginTop: '1rem' }}>
<span style={{ fontSize: '1.5rem' }}>2<span style={{ fontWeight: 'bold', marginLeft: '0.75rem' }}>전역 변수와 전역 객체</span></span>
<br />
아래 코드의 출력 결과는 무엇일까요?
...
`var`로 선언한 전역 변수만 전역 객체(`window`)의 프로퍼티로 등록됩니다. 따라서 `'a' in window`는 `true`가 되지만, 블록 스코프에 묶이는 `let b`와 `const c`는 전역 객체에 올라가지 않아 각각 `false`가 됩니다.
...
<div style={{ border: '1px solid #ccc', padding: '1.25rem', borderRadius: '0.5rem', marginTop: '1rem' }}>
<span style={{ fontSize: '1.5rem' }}>3<span style={{ fontWeight: 'bold', marginLeft: '0.75rem' }}>Call Stack</span></span>
<br />
아래 코드 실행 과정에서 Call Stack은 어떻게 변경될까요?
또한, 주석을 해제한다면 Call Stack이 어떻게 변경될까요?
...
1. `c()` 호출 후
   - `[GLOBAL, A, B, C]`
1. 다시 `c()` 호출
   - `[GLOBAL, A, B, C, C]` -> `[GLOBAL, A, B, C, C, C]` -> `[GLOBAL, A, B, C, C, C]` ... 무한 루프도
1. 결국 Stack이 가득 차고, `Maximum call stack size exceeded` 에러가 발생합니다.
...
<div style={{ border: '1px solid #ccc', padding: '1.25rem', borderRadius: '0.5rem', marginTop: '1rem' }}>
<span style={{ fontSize: '1.5rem' }}>4<span style={{ fontWeight: 'bold', marginLeft: '0.75rem' }}>식별자 검색</span></span>
<br />
`bar()` 내부의 `console.log(x, y, z);` 실행 시, 각 `x`, `y`, `z`에 대해 어떤 렉시컬 환경에서 식별자를 검색할까요?
만약, `const x = 10;`을 주석 처리한다면, 어떻게 될까요?
...
- `x`
  - `bar` 함수 렉시컬 환경에서 식별자 `x`를 검색 -> 없다!
  - `foo` 함수 렉시컬 환경에서 식별자 `x`를 검색 -> 찾았다!
- `y`
  - `bar` 함수 렉시컬 환경에서 식별자 `y`를 검색 -> 없다!
  - `foo` 함수 렉시컬 환경에서 식별자 `y`를 검색 -> 찾았다!
- `z`
  - `bar` 함수 렉시컬 환경에서 식별자 `z`를 검색 -> 찾았다!

---

만약, `const x = 10;`을 주석 처리한다면?

- `x`
  - `bar` 함수 렉시컬 환경에서 식별자 `x`를 검색 -> 없다!
  - `foo` 함수 렉시컬 환경에서 식별자 `x`를 검색 -> 없다!
  - 전역 렉시컬 환경에서 식별자 `x`를 검색 -> 찾았다!
...
<div style={{ border: '1px solid #ccc', padding: '1.25rem', borderRadius: '0.5rem', marginTop: '1rem' }}>
<span style={{ fontSize: '1.5rem' }}>5<span style={{ fontWeight: 'bold', marginLeft: '0.75rem' }}>Block Scope</span></span>
<br />
아래 코드의 실행 결과와, LexicalEnvironment의 변화를 설명해봅시다.
...
`if` 블록 실행 전까지, `LexicalEnvironment`와 `VariableEnvironment` 모두 전역 렉시컬 환경을 참조합니다.

`if` 블록이 실행되면, 새로운 `LexicalEnvironment`가 생성되어 대체되고, `VariableEnvironment`는 여전히 전역 렉시컬 환경을 참조합니다.
새로 생긴 `LexicalEnvironment`의 `OuterLexicalEnvironmentReference`는 전역 렉시컬 환경이 됩니다.

그래서 `if` 블록 내부에서 `let x = 10;`와 `const y = 20;`이 선언되면, `LexicalEnvironment`에 `x`와 `y`가 등록되고, `VariableEnvironment`에는 등록되지 않습니다.
만약 `if` 블록 내부에서, `var`로 변수를 선언한다면 `VariableEnvironment`에만 등록되겠죠?
...
<div style={{ border: '1px solid #ccc', padding: '1.25rem', borderRadius: '0.5rem', marginTop: '1rem' }}>
<span style={{ fontSize: '1.5rem' }}>6<span style={{ fontWeight: 'bold', marginLeft: '0.75rem' }}>복습?</span></span>
<br />
아래 두 코드의 차이가 뭘까요?

<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1.25em' }}>
<div>
...
- `for (let i = 0; ...)`는 반복될 때마다 새로운 `LexicalEnvironment`가 생성됩니다.
  - 콜백 함수가 생성 시점의 `LexicalEnvironment`를 상위 스코프로 가져가는데, 조금 더 자세한 건 클로저에서..
  - 그래서, 각 콜백 함수가 실행될 때, 서로 다른 `LexicalEnvironment`의 `i`값을 참조하게 됩니다.
  - `0 1 2`가 출력됩니다.
- `for (var i = 0; ...)`는 반복될 때마다 같은 실행 컨텍스트 내부의 `VariableEnvironment`에 바인딩된 `i`값을 참조하게 됩니다.
  - 그래서 각 콜백도 같은 `i` 바인딩을 참조하게 됩니다.
  - 루프가 끝나는 시점에서 `i === 3`이니까
  - `3 3 3`가 출력됩니다.
