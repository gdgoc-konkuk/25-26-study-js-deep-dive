...
<details>
  ...
  <strong>정답: ③</strong>
  <br />
  브라우저는 HTML을 파싱해 DOM을 생성하고, CSS를 파싱해 CSSOM을 생성한 뒤 DOM과
  CSSOM을 결합해 Render Tree를 만든다. 이후 Layout(레이아웃 계산)과
  Paint(그리기)를 거쳐 화면을 렌더링한다.
</details>
...
<details>
  ...
  <strong>정답: ③</strong>
  <br />
  JavaScript는 DOM을 수정할 수 있으므로, 브라우저는 HTML 파싱 도중 script 태그를
  만나면 DOM 일관성을 보장하기 위해 파싱을 일시 중단하고 JavaScript를 실행한다.
</details>
...
④ 기본 `<script>` 태그는 HTML 파싱을 중단시키지만, `defer`는 그렇지 않다.
